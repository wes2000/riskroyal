...
func test_send_arity_4_passes_through():
	# Sub-project #7 Plan A Task 10: callv rewrite removed the arity cap.
	# 4-arg calls now pass through correctly (old behavior was push_error + drop).
	var fake = FakeMultiplayerNode.new()
	var sender = MatchRpcSender.new(fake)
	sender.send("test_method", [1, 2, 3, 4])
	assert_eq(fake.rpc_calls.size(), 1, "4-arg send must pass through (no drop)")
	assert_eq(fake.rpc_calls[0].args, [1, 2, 3, 4], "all 4 args preserved")
